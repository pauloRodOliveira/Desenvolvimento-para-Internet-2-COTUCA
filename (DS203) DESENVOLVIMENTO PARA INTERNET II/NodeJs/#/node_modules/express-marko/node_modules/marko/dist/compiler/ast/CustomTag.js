...
class CustomTag extends HtmlElement {
  ...
  buildInputProps(codegen, additionalAttrs) {
    ...
    this.forEachAttribute((attr, i) => {
      ...
      if (attr.spread) {
        let isFirstOfMany = i === 0 && (this._hasDynamicNestedTags || this.attributes.length > 1 || additionalAttrs);
  ...
  getNestedTagVar(context) {
    if (!this._nestedTagVar) {
      let tagDef = this.tagDef;
      let builder = context.builder;
      this._nestedTagVar = builder.identifier(context.nextUniqueId(`nestedTag${tagDef.name}`));
    }
  ...
  generateRenderNode(codegen, tagDef, inputProps, parentCustomTag, renderBody) {
    ...
    if (tagDef.template) {
      ...
      if (isNestedTag) {
        ...
        tagArgs = [builder.identifierOut(), this.tagNameExpression, inputProps.type === "ObjectExpression" && !inputProps.properties.length ? builder.literalNull() : builder.functionDeclaration(null, null, builder.returnStatement(inputProps)), renderBody || builder.literalNull(), argumentNode, properties ? builder.objectExpression(Object.keys(properties).map(propName => {
  ...
  generateCode(codegen) {
    ...
    let additionalAttrs = renderBody && !isDynamicTag && {
      [tagDef.bodyFunction && tagDef.bodyFunction.name || "renderBody"]: renderBody
    };
    let inputProps = this.buildInputProps(codegen, additionalAttrs);
    let renderTagNode = this.generateRenderNode(codegen, tagDef, inputProps, parentCustomTag, renderBody);
